...
# Use specific game and play
GAME_ID = '2023091001'
PLAY_ID = 4239

# Get the play data
selected_play = df_b[
    (df_b['game_id'] == GAME_ID) & (df_b['play_id'] == PLAY_ID)
]

if len(selected_play) == 0:
    print(f"ERROR: Play {PLAY_ID} in Game {GAME_ID} not found!")
    print("Available plays in this game:")
    print(df_b[df_b['game_id'] == GAME_ID]['play_id'].unique())
    sys.exit()

selected_play = selected_play.iloc[0]
...
def update(frame_idx):
    ...
    # 4. Run Models and Calculate Attention Scores
    attention_scores = {}  # Dictionary to store attention scores per node
    ...
    if edge_index.shape[0] == 2 and edge_index.shape[1] > 0:
        ...
        # Calculate attention scores for nodes connected to targeted receiver
        if targeted_receiver_pos is not None:
