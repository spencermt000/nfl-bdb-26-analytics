"""
Dataframe A: Node-Level Features (v3 - With Outputs Data)
=========================================================
...
INPUTS:
  ...
  - data/train/output_*.csv (actual final player positions)
...
OUTPUTS:
  - outputs/dataframe_a/v3.parquet
...
FEATURES ADDED:
  ...
  - Actual final positions from outputs data (output_x, output_y)
  - Distance from current position to final position

CHANGELOG v2 -> v3:
  - Added outputs data integration (actual final player positions)
  - Added output_x, output_y columns
  - Added dist_to_final_pos metric
  - Updated to v3.parquet output
"""
...
print("=" * 80)
print("DATAFRAME A (v3): NODE-LEVEL FEATURES [WITH OUTPUTS DATA]")
print("=" * 80)
print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")

# File paths
INPUT_TRACKING = 'data/train/2023_input_all.parquet'
OUTPUT_TRACKING_PATTERN = 'data/train/output_*.csv'  # Outputs data (actual final positions)
...
print("\nLoading outputs data (actual final positions)...")
import glob
output_files = sorted(glob.glob(OUTPUT_TRACKING_PATTERN))
if output_files:
    ...
    if output_dfs:
        outputs_df = pd.concat(output_dfs, ignore_index=True)
        print(f"  ✓ Loaded {len(outputs_df):,} rows from outputs data")
        
        # Rename columns to distinguish from input tracking
        ...
        outputs_df = outputs_df[['game_id', 'play_id', 'nfl_id', 'frame_id', 'output_x', 'output_y']]
        ...
        print(f"  ✓ Merged outputs data: {len(input_df):,} rows (same: {len(input_df) == initial_len})")
        
        # Calculate distance to final position
        input_df['dist_to_final_pos'] = np.sqrt(
            (input_df['x'] - input_df['output_x'])**2 + 
            (input_df['y'] - input_df['output_y'])**2
        )
        
        outputs_matched = input_df['output_x'].notna().sum()
        outputs_pct = 100 * outputs_matched / len(input_df)
        print(f"  Outputs matched: {outputs_matched:,} / {len(input_df):,} ({outputs_pct:.1f}%)")
        
        del outputs_df, output_dfs
        gc.collect()
    else:
        print("  ⚠ Warning: No output files could be loaded")
        input_df['output_x'] = np.nan
        input_df['output_y'] = np.nan
        input_df['dist_to_final_pos'] = np.nan
else:
...
df_a_cols = [
    ...
    # Standardized tracking data (input positions)
    'x', 'y', 's', 'a', 'dir', 'o',
    
    # Actual final positions (from outputs data)
    'output_x', 'output_y', 'dist_to_final_pos',
