...
class PassPlayGraphDataset(Dataset):
    ...
    def __init__(self, df_a, df_b, df_c, node_features, edge_features, 
        ...
        """
        Args:
            df_a: Node-level dataframe (filtered)
            df_b: Play-level dataframe
            df_c: Edge-level dataframe (filtered)
            node_features: List of node feature column names
            edge_features: List of edge feature column names
            node_means/stds: For normalization
            edge_means/stds: For normalization
        """
        self.df_a = df_a
        self.df_b = df_b
        self.df_c = df_c
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            dict with:
                ...
                - edge_features: [N_edges, edge_feature_dim]
                - ball_progress: scalar
                - n_nodes: int
        """
        ...
        edge_index = torch.LongTensor(edge_index).T  # [2, N_edges]
...
class MultiHeadGraphAttention(nn.Module):
    ...
    def forward(self, node_feat, edge_index, edge_feat, ball_progress):
        ...
        # For edge (i, j): score = Q_i · K_j + edge_features
        ...
        # Softmax per source node (normalize over all edges from same source)
        # This is tricky - need to group by source node
        ...
        # Aggregate messages
        # For each destination node, sum weighted values from sources
...
class AttentionAdjacencyModel(nn.Module):
    """
    Full model: Multi-head attention → Output heads
    """
    ...
    def __init__(self, node_in_dim, edge_in_dim, hidden_dim, n_heads=4):
        ...
        # Prediction heads (for now, just placeholder)
        # TODO: Add completion prediction head, EPA head, etc.
    ...
    def forward(self, node_feat, edge_index, edge_feat, ball_progress):
        ...
        Returns:
            predictions: dict with completion_prob, etc.
            attention_weights: [N_edges, n_heads]
        """
        # Get node embeddings and attention weights
        h, attn_weights = self.attention(node_feat, edge_index, edge_feat, ball_progress)
...
for epoch in range(N_EPOCHS):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        # Skip if no edges (shouldn't happen but just in case)
        ...
        # TODO: Get actual completion label from data
        # For now, use dummy target (will fix later)
        target = torch.rand(1, 1).to(DEVICE)  # Placeholder
        
        # Compute loss
        loss = criterion(predictions['completion_prob'], target)
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            # Dummy target (will fix later)
            target = torch.rand(1, 1).to(DEVICE)
