...
def run_dataframe_a():
    ...
    result = subprocess.run(
        ['python', '/opt/airflow/dags/dataframe_a_v2.py'],
        capture_output=True,
        text=True
    )
...
def run_dataframe_b():
    ...
    import subprocess
    result = subprocess.run(
        ['python', '/opt/airflow/dags/dataframe_b_v2.py'],
        capture_output=True,
        text=True
    )
...
def run_dataframe_c():
    ...
    import subprocess
    result = subprocess.run(
        ['python', '/opt/airflow/dags/dataframe_c_v2.py'],
        capture_output=True,
        text=True
    )
...
def run_dataframe_d():
    ...
    import subprocess
    result = subprocess.run(
        ['python', '/opt/airflow/dags/dataframe_d.py'],
        capture_output=True,
        text=True
    )
...
def run_train_attention():
    """Execute train_att_adj.py"""
    print("\n" + "=" * 80)
    print("RUNNING TRAIN ATTENTION MODEL")
    print("=" * 80 + "\n")
    
    import subprocess
    result = subprocess.run(
        ['python', '/opt/airflow/dags/train_att_adj.py'],
        capture_output=True,
        text=True
    )
    ...
    if result.returncode != 0:
        print("ERROR:", result.stderr)
        raise Exception(f"Train attention failed with return code {result.returncode}")
    
    output_file = 'model_outputs/attention/best_model.pth'
    if not os.path.exists(output_file):
        raise FileNotFoundError(f"Expected output not found: {output_file}")
    
    print(f"\n✓ Training complete: {output_file}")
    return output_file
...
def generate_summary_report(**context):
    ...
    outputs = {
        ...
        'Dataframe B': 'outputs/dataframe_b/v2.parquet',
        'Dataframe B (Pilot)': 'outputs/dataframe_b/v2_pilot_3games.parquet',
        'Dataframe C': 'outputs/dataframe_c/v2.parquet',
        'Dataframe C (Pilot)': 'outputs/dataframe_c/v2_pilot_3games.parquet',
        'Dataframe D': 'outputs/dataframe_d/v2.parquet',
    }
...
# Task 7: Train Attention Model
train_attention_task = PythonOperator(
    task_id='run_train_attention',
    python_callable=run_train_attention,
    dag=dag,
)
...
# Old dependencies stay the same, then add:
[df_c_task, df_d_task] >> train_attention_task >> summary_task
